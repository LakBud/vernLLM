...
function makeFakeAnthropicClient(responseText: string, usage = { input_tokens: 10, output_tokens: 5 }) {
...
describe('fromAnthropic', () => {
  ...
  it('returns content in the chat.completions.create shape', async () => {
    ...
    const result = await adapted.chat.completions.create(
      { model: 'claude-x', temperature: 0.2, max_tokens: 10, messages: [{ role: 'user', content: 'hi' }] },
