 ...
 * strips them, returning a plain VernLLMMiddleware[]. Every `before`
 * normalizes to the equivalent `after` edge on the later entry first, so
 * both directions rank the same way. Each entry's priority is one more
 * than the largest priority among what it must run after, so a chain of
 * dependencies gets strictly increasing numbers instead of collapsing to
 * the same one. Throws on a cycle or an unknown middleware name, naming
 * the offending middleware either way.
 */
function resolveOrder(entries: OrderedMiddleware[]): VernLLMMiddleware[] {
  const byName = new Map(entries.map((entry) => [entry.name, entry]));

  // Normalize every `before` into the equivalent `after` edge on the
  // later entry, so ranking only ever has to walk one kind of edge.
