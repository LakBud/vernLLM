...
<Callout type="info">
  Just want to turn logs off? See [Silencing logs](/docs/core/logging#silencing-logs) for the
  `logger: 'silent'` shorthand.
</Callout>
...
The same logger reaches any circuit breaker or rate limiter adapter that implements `setLogger`, so
their background failures follow your logger and `logger: 'silent'` too.
