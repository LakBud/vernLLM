...
<Callout type="info">
  The Redis example below is illustrative, showing the adapter shape.
  [vern-llm-redis](/docs/integrations/redis) is the production Redis backed cache.
</Callout>
...
<Callout>
  ...
  optional `maxSize` limit to prevent unbounded memory growth. When the limit is reached it evicts
  by `'fifo'` (default) or `'lru'`, see [Eviction](/docs/core/caching#eviction) in Core. Custom
  adapters should keep the same `ttl` convention (seconds, not milliseconds) so behavior stays
