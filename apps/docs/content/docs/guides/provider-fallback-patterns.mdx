...
[Provider Fallback](/docs/core/provider-fallback) covers the `fallback` option itself,
...
```ts title="provider-fallback-patterns-setup.ts"
import Anthropic from '@anthropic-ai/sdk';
import OpenAI from 'openai';
import { VernLLM, fromAnthropic, fromOpenAICompatible } from 'vern-llm';
...
<Callout type="info">
  ...
  jitter](/docs/core/retries#backoff-and-jitter) for how the per-attempt delay is computed.
</Callout>
...
<Callout type="warn">
  ...
  See [circuit_state](/docs/core/observability#circuit_state).
</Callout>
