...
`result.meta` doesn't carry token counts. There are two ways to get real usage into
`cost-tracking`, and which one fits depends on whether usage tracking should live inside the
middleware itself or stay a separate app-level concern.

**Inside the middleware, via its own `onEvent`.** `'usage'` is part of the same event union as
`'retry'`/`'fallback'`/etc, so `cost-tracking` can read real spend without the app wiring
anything extra:
...
This keeps `cost-tracking` self contained: registering it is enough, nothing else in
`VernLLMOptions` needs to change for token cost to start flowing. See [Usage and
usage_failure](/docs/core/observability#usage-and-usage_failure).

**At the app level, via the top-level `onUsage` option**, when usage tracking is a concern the app
owns rather than something bundled into the middleware itself:
...
Both fire from the same underlying event, independently, so using one doesn't preclude the other:
an app can keep its own `onUsage` for billing while a separately authored middleware reads the
identical `'usage'` event for its own tracing, neither aware the other exists. `onUsage`/`'usage'`
fires per real provider response; `wrap`'s `next()` resolves once per logical call. Correlate the
two by `requestId` for per-attempt token cost instead of just the logical call's total elapsed
time.
