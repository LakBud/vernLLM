...
- **`jsonSchema` implies JSON mode.** It overrides `jsonMode: false` and sends the request as structured JSON.
  **`strict` is forwarded when supported by the adapter.** Enforcement depends on the adapter and the mechanism it uses: OpenAI/Groq use JSON Schema mode, Gemini uses schema-based structured output without relying on `strict`. On Anthropic and Bedrock, this only applies to the legacy forced-single-tool-call mechanism (Anthropic's tool strictness, Bedrock's tool strictness where supported); their native structured-output mechanism (see <a href="#combining-with-tools">Combining with tools</a>) has no `strict` field at all, on either provider.
