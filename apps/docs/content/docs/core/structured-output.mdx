...
<Callout>
  ...
  sent through `output_config.format` instead (see{' '}
  <a href="#combining-with-tools">Combining with tools</a>). A client-side `schema` is still
  recommended when you need validation guarantees across providers, regardless of which path is
  used.
</Callout>
...
sent. Gemini and OpenAI-compatible clients never had this restriction and don't need
