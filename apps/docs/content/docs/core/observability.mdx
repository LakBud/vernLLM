...
rather than something that happened to one. See <a href="/docs/API-reference/configuration#provider-identity">Provider identity</a>.
...
The meaning of `consecutiveFailures` and the effect of `isolateByModel` are covered in <a href="/docs/core/circuit-breaker">Circuit Breaker</a>.
...
Emitted when a `fallback` target's own retries are exhausted or abandoned and the chain moves on
to the next target. Not emitted when a lone target (no `fallback` configured) fails, since there's
nowhere for the chain to move to.
...
See <a href="/docs/core/provider-fallback">Provider Fallback</a> for the `fallback` option itself
and <a href="/docs/guides/provider-fallback-patterns#turn-onevents-fallback-event-into-an-alert-not-just-a-log-line">turning
this event into an alert</a> in the companion guide.
