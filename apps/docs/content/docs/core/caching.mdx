...
<Callout type="warn">
  Cache adapters should use the `hit` field to indicate whether a value exists. Returning
  `{ hit: true, value: null }` is valid, while `{ hit: false, value: null }` indicates a cache miss.
</Callout>
