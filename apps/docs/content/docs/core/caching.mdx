...
<Callout>
  The built-in `InMemoryCacheAdapter` treats `ttl` as seconds. Keep custom adapters consistent with
  that convention (rather than milliseconds) so `ttl` means the same thing regardless of which
