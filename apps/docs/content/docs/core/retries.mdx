...
Every option on this page governs VernLLM's own retry loop, the one wrapping `client.chat.completions.create()` (or the equivalent per adapter). It has no visibility into what happens _inside_ that call. `fromOpenAI`, `fromAnthropic`, `fromGemini`, and `fromBedrock` each wrap an official provider SDK, and every one of those SDKs retries transient failures, including 429s, on its own, by default, before VernLLM's `create()` call ever returns:

| Adapter         | SDK                 | Default internal retry                           |
| --------------- | ------------------- | ------------------------------------------------ |
| `fromOpenAI`    | `openai`            | `maxRetries: 2`                                  |
| `fromAnthropic` | `@anthropic-ai/sdk` | `maxRetries: 2`                                  |
| `fromGemini`    | `@google/genai`     | Automatic backoff on 429/5xx, on by default      |
| `fromBedrock`   | AWS SDK v3          | Standard retry mode, `maxAttempts: 3` by default |

`fromFetch` is the one exception: it calls `fetch` directly, so VernLLM's own retry loop is the only one in play there.
