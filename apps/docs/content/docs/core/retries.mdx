...
Every normalized `LLMError` carries an optional `attempts` array: one entry per attempt made against the current target before the error was thrown, each with the attempt's `index` and its own normalized `error`.
