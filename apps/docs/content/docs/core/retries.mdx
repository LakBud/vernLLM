...
A negative or NaN `maxRetries` means no retries, so the call still makes one attempt, and a warning is
logged. A fraction rounds down.
...
- `error.type` is `parse` or `validation`, since these are deterministic response processing failures that a retry will not fix. The exception is `response_truncated`: output cut off at `max_tokens` is retried, since a resend can finish in time
...
- **Blind telemetry.** `onEvent`'s `retry` events, the logger, and budget reservation all see nothing while a retry happens at the SDK layer.
...
| `nonRetryableStatus` | `[400, 401, 402, 403, 404, 413, 422]` | Status codes that fail immediately instead of retrying.                                                                  |
