...
<Callout>
  `onUsage` is purely observational and fires after a call succeeds — it can't prevent a call from
  happening. If you need to enforce a quota or limit *before* a call is dispatched, see
