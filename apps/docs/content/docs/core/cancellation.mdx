...
Each attempt gets its own internal timeout controller. If you also pass a `signal`, vern-llm combines both into a single signal via `AbortSignal.any([yourSignal, internalTimeoutSignal])` — either one firing cancels the in-flight request. The internal timer is always cleared afterward, whether the call succeeds, fails, or is aborted, so nothing is left running in the background.
