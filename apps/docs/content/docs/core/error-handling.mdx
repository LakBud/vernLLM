...
<Cards>
  ...
  <Card title="api">
    ...
    `nonRetryableStatus` (default `400, 401, 402, 403, 404, 413, 422`).
  </Card>
  <Card title="network">
  ...
  <Card title="quota_exceeded">
    ...
    Breaker](/docs/core/circuit-breaker#what-counts-as-a-failure).
  </Card>
...
triggers each of these. See [Retry Budget](/docs/core/retry-budget) for `retry_budget_exhausted`
specifically.
...
The currently defined parsing code is:

| Code                 | Error type | Meaning                                                                                                                                                         |
| -------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `response_truncated` | `parse`    | The provider stopped at `max_tokens` and the output couldn't be parsed. Retryable, and never counts toward the breaker. The original parse error is on `cause`. |
...
```ts title="soft-failure-setup.ts"
const llm = new VernLLM({
  client: fromOpenAI(openai),
  model: 'gpt-4o',
  detectSoftFailure: (result, meta) => {
    // meta: { requestId, model, providerName, isFallback, attempt, usage }
...
`meta.usage` carries the same `TokenUsage` `onUsage` would report for this attempt, letting a hook
reclassify a response as a failure based on real spend rather than content alone:

```ts title="soft-failure-cost-cap.ts"
const llm = new VernLLM({
  client: fromOpenAI(openai),
  model: 'gpt-4o',
  detectSoftFailure: (result, meta) => {
    if (meta.usage && meta.usage.totalTokens > 40_000) {
      return 'soft_failure_detected';
    }
    return undefined;
  },
...
`meta.usage` is `undefined` when the provider omitted usage on that response, not when usage was
zero, so a cost check should treat a missing value as unknown rather than as free.
...
See [Retries](/docs/core/retries) and [Circuit Breaker](/docs/core/circuit-breaker).
...
<Callout type="info">
  ...
  [`retryable`](#retryable) below). A code excluded from breaker accounting, such as
...
- `type` is `'parse'`, `'validation'`, `'invalid_params'`, or `'aborted'`, except `code:
'response_truncated'`, which is retryable
...
- `code` is one of the local rate limit codes (`rate_limit_queue_full`,
  `rate_limit_queue_timeout`, `rate_limit_capacity_exceeded`, `retry_budget_exhausted`)
...
<Callout type="info">
  ...
  [Circuit Breaker](/docs/core/circuit-breaker#what-counts-as-a-failure) for the full list of what's
...
- `error.type` is `'parse'`, `'validation'`, or `'invalid_params'` because these are deterministic
  input or response processing failures rather than transient provider faults. `response_truncated`
  is the exception and is retried
...
- `error.code` is `'retry_budget_exhausted'` because a configured `retryBudget` decided this
  target has already had too many retries recently; further retrying against it is refused until
  the trailing window's ratio drops. See [Retry Budget](/docs/core/retry-budget).
- `error.status` is in `nonRetryableStatus` (default `[400, 401, 402, 403, 404, 413, 422]`)
