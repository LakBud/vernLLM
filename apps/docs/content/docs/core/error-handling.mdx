...
<Callout type="warn">
  Unlike the non-streaming path, a soft failure detected on a streaming call does not count toward
  the circuit breaker. By the time the final result is shaped, the streaming attempt has already
  returned successfully from VernLLM's own retry loop, so nothing is still listening for this
  failure the way the non-streaming path is. Only a genuine mid-stream failure, such as the
  connection going idle, is recorded against the breaker for a streaming call.
</Callout>
