...
`LLMError.toolIssues` contains the complete set of detected issues.
...
```ts title="handling-tool-issues.ts"
try {
  await llm.call({
    userContent: 'Use the available tools.',
    tools: [weatherTool],
...
The top level `code` identifies the primary contract error. `toolIssues` contains the complete set
of detected contract problems so the application can diagnose them together.
...
`unknown_tool` and `duplicate_tool_call_id` are deliberately non retryable.

An `unknown_tool` error means the model requested a tool that was not offered in the request.
Retrying the same request does not change the available tool contract.

A `duplicate_tool_call_id` error means the model returned multiple tool calls with the same ID.
Retrying is not useful for repairing the invalid response contract.

These errors therefore fail immediately instead of consuming the remaining `maxRetries`.
