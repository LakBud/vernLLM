...
| Code                        | Error type       | Meaning                                                                                                                                                                                                                                                    |
| --------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `unsupported_capability`    | `invalid_params` | The caller asked for something this specific adapter, client, or model can't do: `stream: true` against a client with no streaming method, a model not covered by `toolUseSupportedModels`, or `toolChoice: 'none'` against a provider with no equivalent. |
...
throw errors with circular references. `JSON.stringify(err)` itself is always safe: `LLMError`
leaves `cause` out of its own serialization, along with the structured
`type`/`code`/`status`/`issues` fields it does include. It also adds `message` and `retryable`,
since a plain `Error` subclass without this wouldn't show either: `message` is non-enumerable on
`Error`, and `retryable` is a getter, not an own property. `err.attempts` works the same way and
never carries `cause`.
