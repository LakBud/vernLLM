...
<Callout type="info">
  ...
  Even though the streaming attempt has already returned successfully from VernLLM's own retry loop
  by the time the final result is shaped, a finalize-time failure (including one from
  `detectSoftFailure`) is still recorded against the breaker directly.
</Callout>
