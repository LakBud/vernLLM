...
<Cards>
  ...
  <Card title="api">
    ...
    `unknown_tool`, `duplicate_tool_call_id`, or `tool_choice_none_violated`, and are not retried.
...
| `tool_choice_none_violated` | `api`      | The provider returned `tool_calls` despite `toolChoice: 'none'`. See [Tool Calling](/docs/core/tool-calling). |

The currently defined rate-limiting codes are:

| Code                    | Error type       | Meaning                                                                                                                                                                                                                                            |
| ----------------------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `local_rate_limit`      | `quota_exceeded` | A `rateLimit` call was rejected locally, without reaching the provider: the queue wait exceeded `maxQueueMs`, the queue was already at `maxQueueSize`, or the call's estimated tokens exceeded the configured `tokensPerMinute` capacity outright. |
...
The currently defined connectivity codes are:

| Code                  | Error type | Meaning                                                                                                                                                                                                                                                                                     |
| --------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `invalid_credentials` | `api`      | The provider returned HTTP 401 or 403. Set whenever a response, or an already-normalized `LLMError`, carries that status and no more specific `code` is already present, regardless of which adapter or client raised the error.                                                            |
...
| Code                 | Error type                          | Meaning                                                                                                              |
| -------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `fallback_exhausted` | the last failed target's own `type` | The primary target and every attempted fallback target failed. Only ever set on `FallbackExhaustedError`, see below. |
...
`instanceof LLMError` check still passes, and it carries `attempts`, every target's own normalized
error, in the order they were tried:
...
- `error.code` is `'unknown_tool'`, `'duplicate_tool_call_id'`, or `'tool_choice_none_violated'`
  because these are deterministic tool contract failures
- `error.code` is `'local_rate_limit'` because either the wait already happened (retrying would only
  requeue behind the same limit with nothing changed) or the call could never fit the configured
  `tokensPerMinute` capacity in the first place, so no amount of retrying changes that. See [Interaction with retries](/docs/core/rate-limiting#interaction-with-retries).
...
`unknown_tool`, `duplicate_tool_call_id`, and `tool_choice_none_violated` are deliberately non
retryable. See [Tool Calling](/docs/core/tool-calling) for the canonical contract workflow and
handling guidance. These errors therefore fail immediately instead of consuming the remaining
`maxRetries`.
...
<Callout type="warn">
  ...
  `unknown_tool`, `duplicate_tool_call_id`, and `tool_choice_none_violated` cannot cause a healthy
  provider's circuit to open.
</Callout>
