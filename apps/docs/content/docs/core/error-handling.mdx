...
| Code                 | Error type                          | Meaning                                                                               |
| -------------------- | ----------------------------------- | ------------------------------------------------------------------------------------- |
| `fallback_exhausted` | the last failed target's own `type` | Every `fallback` target failed. Only ever set on `FallbackExhaustedError`, see below. |

See <a href="/docs/core/provider-fallback">Provider Fallback</a> for the full `fallback` option
...
tried. See <a href="/docs/core/provider-fallback#when-every-target-fails">When every target fails</a>.
