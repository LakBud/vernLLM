...
<Cards>
  ...
  <Card title="rate_limited">
    A rate limit was hit, either VernLLM's own local `rateLimit` queue or the provider's own HTTP
    429. See [Rate Limiting](/docs/core/rate-limiting). Whether this is retried depends on `code`,
    see below.
  </Card>
...
are exactly the ones whose `message` already string-joins a list a caller might want to consume
directly instead of re-parsing it out of prose; see [`issues`](#issues) below for the typed payload
each one carries.
...
- `type` is `'parse'`, `'validation'`, or `'invalid_params'`
