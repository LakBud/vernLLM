...
<Cards>
  ...
  <Card title="api">
    ...
    `nonRetryableStatus` (default `400, 401, 403, 404, 422`). Also raised, and retried, when the
    model requests a tool name that was not in the `tools` array, on the reasoning that a
    hallucinated tool name may not recur. See <a href="/docs/core/tool-calling">Tool Calling</a>.
  </Card>
  <Card title="parse">
