...
const llm = new VernLLM({
  ...
  onEvent: (event) => {
    if (event.kind === 'fallback') {
      logger.warn(`falling over ${event.from} -> ${event.to}`, { error: event.error });
    }
  },
});
...
const llm = new VernLLM({
  ...
  fallback: {
    ...
    // maxRetries and timeoutMs inherit the primary's: 2 and 15000.
    // Only what's different needs stating.
    timeoutMs: 30_000,
  },
});
```
...
<a href="/docs/core/circuit-breaker#scope">Circuit Breaker</a> and
<a href="/docs/core/rate-limiting">Rate Limiting</a> for their individual behavior.
...
and state inspection, see <a href="/docs/core/circuit-breaker">Circuit Breaker</a>.
...
See <a href="/docs/core/error-handling">Error Handling</a> for the full `LLMError` type and code
...
<Callout type="warn">
  ...
  <a href="/docs/core/streaming">Streaming</a> for the rest of the `stream: true` contract.
</Callout>
...
`instanceof LLMError` check still passes. It carries `attempts`, every target's own normalized
error, in order:
...
try {
  ...
  if (err instanceof FallbackExhaustedError) {
    for (const attempt of err.attempts) {
      logger.error(`${attempt.provider} failed`, {
...
coalescing for concurrent misses on the same `cacheKey` covers the full chain too. See <a href="/docs/core/caching">Caching</a>.
...
See <a href="/docs/API-reference/configuration#fallback">Configuration</a> for how `fallback` sits
alongside every other constructor option, and

<a href="/docs/guides/provider-fallback-patterns">Provider Fallback Patterns</a> in Guides for
