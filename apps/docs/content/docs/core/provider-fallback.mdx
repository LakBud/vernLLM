...
If your primary provider fails, VernLLM tries your backups in the order you declare.
...
and `model`, everything else is optional. A per call `model` override applies to the primary only,
since it names a model on the primary's provider. Fallback targets always run their own `model`. Retry and timeout knobs left unset inherit the parent
...
| `type: 'invalid_params'`, except `code: 'unsupported_capability'` | `'stop'` | Caller input rejected locally. Every target rejects it the same way.                                       |
| `code: 'unsupported_capability'`                                  | `'next'` | One adapter's limit. Another target may support it.                                                        |
