...
- `openai` is the only formal (optional) peer dependency. `zod` and other provider SDKs
  (`groq-sdk`, `@anthropic-ai/sdk`, etc.) are not runtime dependencies. This package only relies on
  their shapes structurally, so bring your own.
