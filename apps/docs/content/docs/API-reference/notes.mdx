...
- VernLLM does not require a specific provider SDK. The package operates against the `LLMClient` interface, so provider SDKs are brought by the consumer or accessed through adapters.
- Validation libraries such as `zod` are optional. If using `schema`, provide a Zod-compatible validator exposing `safeParse`.
