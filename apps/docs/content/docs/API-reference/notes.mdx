...
- `tools` and `schema` are never mutually exclusive, on any provider; `schema` only validates parsed JSON text and never touches request fields tool calling uses. `tools` and `jsonSchema` are mutually exclusive by default on Anthropic and Bedrock only, and throw `LLMError('validation')` if combined there, except on models opted into `nativeStructuredOutputModels`. Gemini and OpenAI-compatible clients never restrict `tools` + `jsonSchema` at all. See [Structured Output](/docs/core/structured-output#combining-with-tools).
