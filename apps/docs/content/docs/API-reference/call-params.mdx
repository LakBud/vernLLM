...
| ----------------- | -------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`           | `string`                                           | Instance `model`                                      | Overrides the model for this call. Applies to the primary only, fallback targets keep their own.                                                                                                                                            |
...
<Cards>
  ...
  <Card title="meta">
    ...
    `call()` resolves, for `stream: true` that's once `call()` returns `{ chunks, finalResult }`,
    not once the stream finishes. Build one with the `metaRef()` helper instead of writing the
    type out by hand. See
    [Knowing which target answered](/docs/core/provider-fallback#knowing-which-target-answered).
  </Card>
</Cards>

For `cachedCall`, each caller has its own cancellation signal and deadline. Aborting one coalesced
caller, or hitting its `deadlineMs`, does not cancel the shared operation for other callers. The
shared request is aborted once every caller has left.
