...
| `reasoningEffort` | `'minimal' \| 'low' \| 'medium' \| 'high'` | Instance `defaultReasoningEffort`, otherwise not sent | Passed through when set.                                          |
| `budgetTokens`    | `number`                                   | Instance `defaultBudgetTokens`, otherwise not sent    | Native on Anthropic and Gemini. See below.                        |
