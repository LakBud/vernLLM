...
description: Erm, its the LLM call framework for resilience, observability, and control
icon: CircleQuestionMark
---

## Get production-grade LLM calls, without any bloat

**VernLLM** is the LLM call framework. Resilience, observability, and control for every call. A lightweight wrapper that adds retries, timeouts, circuit breakers, provider fallback, rate limiting, caching, streaming, structured outputs, schema validation, usage tracking, and observability events with sensible defaults and minimal dependencies.
