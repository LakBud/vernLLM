...
<Callout>
  `jsonSchema` uses Gemini's native `responseSchema` + `responseMimeType: 'application/json'`. The
  adapter forwards the schema and description into `generationConfig.responseSchema`. Gemini
  enforces the provided schema without a separate `strict` flag. `reasoning_effort` is dropped
