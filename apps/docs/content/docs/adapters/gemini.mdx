...
`fromGemini` uses the function name as the wire tool call id, adding a `#n` suffix on the second and
later calls to the same tool within one turn so they stay distinguishable; the suffix is stripped
back off before a tool result is sent as a `functionResponse`. Consecutive function response
