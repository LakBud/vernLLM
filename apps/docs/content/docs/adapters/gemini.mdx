...
When Gemini omits it, `fromGemini` falls back to the function name as the wire tool call id, adding
a `#n` suffix on the second and later calls to the same tool within one turn so they stay
distinguishable. That synthesized id is never sent back to Gemini; the bare function name is sent
on the `functionResponse` instead. Consecutive function response messages from a multi-tool turn are
merged back into a single `user` content entry. See [Tool Calling](/docs/core/tool-calling) for the
full reference.
