 ...
 * trading a clear `LLMError('invalid_params')` for a confusing error from
 * the provider instead. So this is opt-in: pass the model IDs you've
 * verified against the provider's own docs (or a predicate). Left unset,
 * no model is treated as native-capable, `jsonSchema` keeps using the
 * older forced single tool call emulation, and `tools` + `jsonSchema`
 * together is rejected, exactly this package's behavior before native
 * support was added.
 */
