...
describe('isStreamResult()', () => {
  ...
  it('returns false when chunks is not an async iterable (regression: an object with no Symbol.asyncIterator)', () => {
    expect(
      isStreamResult({ chunks: { not: 'iterable' }, finalResult: Promise.resolve('hi') }),
    ).toBe(false);
  });

  it('returns false when chunks is null', () => {
    expect(isStreamResult({ chunks: null, finalResult: Promise.resolve('hi') })).toBe(false);
  });

  it('returns false when chunks is missing entirely', () => {
    expect(isStreamResult({ finalResult: Promise.resolve('hi') })).toBe(false);
  });

  it('returns false when chunks.Symbol.asyncIterator is not a function', () => {
    const notIterable = { [Symbol.asyncIterator]: 'nope' };

    expect(isStreamResult({ chunks: notIterable, finalResult: Promise.resolve('hi') })).toBe(false);
  });
});
