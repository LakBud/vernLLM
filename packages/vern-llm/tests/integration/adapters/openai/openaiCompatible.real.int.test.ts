...
/**
 * Exercises `fromOpenAICompatible` (and its `fromGroq` alias, since
 * OpenAI-compatible providers share this adapter) against a *real* `openai`
 * SDK client instance, pointed at a local mock server instead of
 * `api.openai.com`. Unlike `openaiCompatible.int.test.ts` (a hand-rolled
 * fake `{ chat: { completions: { create } } }`), this proves the real SDK's
 * request/response objects actually satisfy the adapter's assumed wire
 * shape.
 */
describe('OpenAI-compatible adapter integration (real openai SDK client)', () => {
  ...
  it('sends real tool_calls round-trip through the real SDK client', async () => {
    server = await startRealSdkServer([
      {
        body: {
          id: 'chatcmpl-2',
    ...
    expect(sent.body).toMatchObject({
      tools: [
        expect.objectContaining({ function: expect.objectContaining({ name: 'getWeather' }) }),
      ],
    });
  });
  ...
  it('surfaces a real OpenAI SDK error (429) through VernLLM retry handling', async () => {
    server = await startRealSdkServer([
      {
        status: 429,
        body: { error: { message: 'Rate limited by mock server', type: 'rate_limit_error' } },
      },
      {
        body: {
          id: 'chatcmpl-3',
          object: 'chat.completion',
          model: 'gpt-test',
          choices: [{ index: 0, message: { role: 'assistant', content: 'ok after retry' } }],
    ...
    server = await startRealSdkServer([
      {
        body: {
          ...
          model: 'gpt-test',
          choices: [
            { index: 0, message: { role: 'assistant', content: 'I see a small red square.' } },
          ],
