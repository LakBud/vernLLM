...
- d02c2a7: Caching, rate limiting, retry, fallback, middleware, and streaming fixes, plus a new `response_truncated` error code.

  Caching: `NormalizedCacheAdapter` no longer strips punctuation or case, so `"2+2"` and `"2-2"` no longer share an answer. It only normalizes Unicode composition, line endings, and outer whitespace. `InMemoryCacheAdapter` copies values on write and read, and doesn't cache a value it can't copy faithfully (a class instance or function anywhere inside it). A missing, NaN, zero, or negative `ttl` stores nothing instead of an entry that never expires. `cachedCall` warns when `ttl` is missing or NaN and skips the write. `TieredCacheAdapter` no longer serves a value after L2 has expired it.
  ...
  Middleware: `ctx.own` persists for one middleware across all its hooks within a logical call. An unnamed middleware is labeled `[i]` everywhere, including `ctx.registeredMiddlewareNames`, and a `name` that matches another entry's `[i]` label throws at construction. The response body in a `fromFetch` error passes through `redact`. A circuit transition with no call behind it reaches middleware `onEvent`.
...
- c45a708: Coalescing, abort, and deadline fixes.
...
- 2565924: Add `requireRef` and `RequiredMiddlewareRef` for mandatory `runsAfter`/`runsBefore` dependencies.
...
- f17e8e0: Add `MiddlewareRef` and `createMiddlewareRef` for `runsAfter`/`runsBefore`.
...
- 1e4f6e8: Add `usage` to `SoftFailureMeta`, the context object passed to `detectSoftFailure`. Carries the same `TokenUsage` `onUsage` would report for the attempt, `undefined` when the provider omitted it on that response, not when usage was zero.
...
- e149039: Add `logger: 'silent'` shorthand to `VernLLMOptions`, backed by an internal no-op logger. Silencing all output no longer requires stubbing `debug`, `warn`, and `error` yourself.
...
- 55a87b3: Add `estimateFraction` to `RateLimitOptions`. Scales the pre-flight token estimate down before it's reserved against `tokensPerMinute`, since most calls don't use their full `max_tokens` budget. Default `1`, matching prior behavior. Never changes the `max_tokens` sent to the provider; reconciliation against real usage on `release` is unaffected.
- 5c4b677: Add `runsAfter`/`runsBefore` and `position` to `VernLLMMiddleware` for composing middleware from more than one source without every author agreeing on the same `priority` numbers ahead of time.
...
- b3a5de4: Added `budgetTokens` on `CallParams`, a numeric reasoning budget alongside the existing
  ...
  [Call Params reference](https://vernllm.dev/docs/API-reference/call-params) for full per
  provider behavior.
...
- 09c330b: Added `fromOpenAI`, an alias of `fromOpenAICompatible` for OpenAI clients. The
  ...
  going forward. See [Migration Notes](https://vernllm.dev/docs/migration-notes) for details.
...
- bc1fc46: Added `onUsageFailure`, an opt-in hook that reports token usage for calls that spent real tokens but then failed on VernLLM's own post-response handling, such as parse or schema validation errors, instead of silently dropping that spend.
  ...
  See the [Usage Tracking](https://vernllm.dev/docs/core/usage-tracking) docs for the full shape and firing semantics.
